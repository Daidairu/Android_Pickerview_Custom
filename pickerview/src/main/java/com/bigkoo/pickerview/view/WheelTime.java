...
public class WheelTime {
     ...
     * 设置公历
     * 她媽我超級大改 幹
     * 所有用 WeekDaysWheelAdapter 的都是我新改的，為了讓日期後面有週一週二顯示
     * 要改回原版就把用 WeekDaysWheelAdapter的都幹掉 改回用 NumericWheelAdapter
     *
     * @param year
     * @param month
     * @param day
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        if (startYear == endYear && startMonth == endMonth) {
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                isShowWeekDayAdapter(year, month, startDay, endDay);
            ...
            // 起始日期的天数控制
            if (list_big.contains(String.valueOf(month + 1))) {
                isShowWeekDayAdapter(year, month, startDay, 31);
            } else if (list_little.contains(String.valueOf(month + 1))) {
                isShowWeekDayAdapter(year, month, startDay, 30);
            } else {
                // 闰年 29，平年 28
                isShowWeekDayAdapter(year, month, startDay, leapYear ? 29 : 28);
            }
            wv_day.setCurrentItem(day - startDay);
            ...
            if (list_big.contains(String.valueOf(month + 1))) {
                ...
                isShowWeekDayAdapter(year, month, 1, endDay);
            } else if (list_little.contains(String.valueOf(month + 1))) {
                if (endDay > 30) {
                    endDay = 30;
                }
            ...
            // 判断大小月及是否闰年,用来确定"日"的数据
            if (list_big.contains(String.valueOf(month + 1))) {
                isShowWeekDayAdapter(year, month, 1, 31);
            } else if (list_little.contains(String.valueOf(month + 1))) {
                isShowWeekDayAdapter(year, month, 1, 30);
            } else {
                // 闰年 29，平年 28
                isShowWeekDayAdapter(year, month, startDay, leapYear ? 29 : 28);
            }
    ...
    /**
     * 決定要用有或沒有顯示星期幾的adapter,
     * WeekDaysWheelAdapter是會顯示的，我自己寫的垃圾；
     * NumericWheelAdapter是不會顯示的原生套件
     */
    private void isShowWeekDayAdapter(int year, int month, int startDay, int endDay) {
        if (isShowWeekDay) {
            wv_day.setAdapter(new WeekDaysWheelAdapter(year, month, startDay, endDay));
        } else {
...
//                maxItem = endD;
    ...
    public void setLabels(String label_year, String label_month, String label_day,
        ...
        /**我自己加的 不這樣加你的星期會變成 "24周四日",所以後面不能加東西才能變成"24周四"
         * 看是要"24周四" 或是 "24日"
         */
        if (isShowWeekDay) {
            wv_day.setLabel("");
